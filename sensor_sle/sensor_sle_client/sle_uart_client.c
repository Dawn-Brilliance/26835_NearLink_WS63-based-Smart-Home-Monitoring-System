...
volatile uint16_t g_active_conn_id = 0xFFFF;
...
void sle_uart_client_init(ssapc_notification_callback notification_cb, ssapc_indication_callback indication_cb)
{
    ...
    if (enable_sle() != ERRCODE_SUCC) {
        osal_printk("[SLE Client] ERROR: SLE enable failed! \n");
        return;
    } else {
        osal_printk("[SLE Client] SLE enabled successfully\n");
    }
}